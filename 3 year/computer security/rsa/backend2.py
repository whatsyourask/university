...
class RSA:
    ...
    def _chr_decrypt(self, seq: str) -> str:
        ...
        num = str(pow(int(seq), self._d, self._n))
        print(num)
        num_list = []
        for i in range(0, len(num), 2):
            num_list.append(int(num[i:i + 2]))
        print(num_list)
        num_list = list(map(lambda num: num + 1024, num_list))
        print(num_list)
        return ''.join(list(map(lambda char: chr(char), num_list)))
